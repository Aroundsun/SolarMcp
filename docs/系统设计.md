...
│    ├── ToolManager::registerTool(内置 read_file 等)              │
│    ├── PluginManager::loadFromDirectory(plugins/lib/)            │
│    │     └── ABI 校验 → init(host_api) → register → CallbackTool │
...
| **插件加载** | dlopen + dlsym + 纯 C ABI | 插件仅依赖 `plugin_abi.h`，宿主通过 `mcp_host_api` 注册工具，不跨 `.so` 传 C++ 对象 |
...
plugins:
  directory: "./plugins/lib/"   # 运行时 .so 目录（源码在 plugins/ 各子目录）
  autoload: true
  allow_reload: true
...
tools:
  ...
  shell:                        # 由 shell_plugin.so 读取（get_config_path）
    enabled: true
    timeout_sec: 30
    max_output_mb: 10
...
**技术方案**: dlopen + dlsym + **纯 C ABI**（`include/mcp/plugin/plugin_abi.h`）

插件与主程序之间的**唯一**二进制契约。插件不得 `#include` 主程序 C++ 头文件（如 `tool_manager.h`），不得跨 `.so` 边界传递 C++ 对象。

```c
// 插件必须导出（extern "C" 或纯 C 编译）
int mcp_plugin_abi_version(void);          // 必须 == SOLARMCP_PLUGIN_ABI (1)
const char* mcp_plugin_version(void);      // 语义版本，如 "2.0.0"
const char* mcp_plugin_name(void);         // 逻辑名，如 "shell_plugin"
int mcp_plugin_init(const mcp_host_api* host);
int mcp_plugin_register(void);
void mcp_plugin_shutdown(void);            // 可选，卸载前清理
```

宿主通过 `mcp_host_api` 向插件提供：

| 回调 | 作用 |
|------|------|
| `register_tool` | 注册 MCP 工具（name、description、input_schema_json、execute 函数指针） |
| `log` | 写服务器日志 |
| `get_config_path` | 返回 `config.yaml` 路径，供插件读取 `tools.*` 配置 |

宿主侧用 `CallbackTool` 将 C 风格 `execute` 回调包装为 `Tool` 并注册到 `ToolManager`。

**加载流程**:

1. 读取 `plugins.directory`（默认 `./plugins/lib/`）
2. `readdir` 扫描 `*.so`（跳过 `*.so.*` 版本化后缀）
3. `dlopen(RTLD_NOW)` → `dlsym` 解析上述导出符号
4. `mcp_plugin_abi_version()` 与 `SOLARMCP_PLUGIN_ABI` 不一致则拒绝加载
5. `mcp_plugin_init(&host_api)` → `mcp_plugin_register()`
6. 失败时 `mcp_plugin_shutdown()` + `dlclose`；成功则记录 handle、工具名列表、插件版本
7. 退出或热重载：`unregisterTool` → `mcp_plugin_shutdown()` → `dlclose`

**热重载（安全）**:

- `reloadPlugin` / `reloadFromDirectory` 先 `validatePlugin`（试加载 + shutdown），全部通过后再卸载旧插件
- validate 失败时保留当前已加载插件，避免半更新

**示例插件**:

| 文件 | 说明 |
|------|------|
| `plugins/example/minimal_plugin.c` | 纯 C 最小示例 |
| `plugins/shell/shell_plugin.cpp` | C++ 业务 + C ABI 桥接 |

详见 [插件ABI.md](./插件ABI.md)、[插件热重载.md](./插件热重载.md)。
...
│   └── plugin/
│       ├── plugin_abi.h                    # 纯 C 插件 ABI 契约
│       ├── plugin_manager.h                # PluginManager dlopen 管理
│       └── callback_tool.h                 # C execute 回调 → Tool 适配
│
├── src/
│   ├── logger/
...
│       └── callback_tool.cpp               # CallbackTool 实现
│
├── plugins/
│   ├── CMakeLists.txt                      # 插件构建
│   ├── README.md                           # 插件目录说明
│   ├── example/
│   │   └── minimal_plugin.c                # 最小插件示例（纯 C ABI）
│   ├── shell/                              # Shell Tool 插件
│   │   ├── shell_tool.h / shell_tool.cpp
│   │   └── shell_plugin.cpp                # C ABI 桥接
│   └── lib/                                # 运行时 .so（构建生成）
│       ├── shell_plugin.so
│       └── minimal_plugin.so
│
...
├── docs/
│   ├── 系统设计.md                    # 本文档
│   ├── 插件ABI.md                     # 纯 C 插件 ABI 设计与编写指南
│   ├── 插件热重载.md                  # 运行时热重载操作指南
│   ├── 类图.mermaid                   # 类图（Mermaid）
│   └── 时序图.mermaid                 # 时序图（Mermaid）
│
├── scripts/
│   └── build.sh                            # 构建脚本
│
...
> 完整的 Mermaid 类图另存为：`docs/类图.mermaid`
...
│  │   （内置，非插件）         │    ├──────────────────────────────┤   │
│  ├───────────────────────────┤    │ - tools_: map<str, uptr<Tool>│   │
│  │ - maxSize_: size_t        │    ├──────────────────────────────┤   │
│  │ - allowedPaths_: vec<str> │    │ + registerTool(uptr<Tool>)   │   │
│  ├───────────────────────────┤    │ + unregisterTool(name)       │   │
│  │ + name(): "read_file"     │    │ + getTool(name): Tool*       │   │
│  │ + execute(params, ctx)    │    │ + listTools(): vector<Info>  │   │
│  └───────────────────────────┘    │ + callTool(name, params, ctx)│   │
│                                   └──────────────────────────────┘   │
│  ┌───────────────────────────┐    ┌──────────────────────────────┐   │
│  │      CallbackTool         │    │       PluginManager          │   │
│  │  （宿主内，包装 C 回调）   │    ├──────────────────────────────┤   │
│  ├───────────────────────────┤    │ - plugins_: vector<Loaded>   │   │
│  │ - execute_fn_: C 函数指针 │    │   (handle, path, name,       │   │
│  │ - input_schema_json_      │    │    version, tool_names)      │   │
│  ├───────────────────────────┤    ├──────────────────────────────┤   │
│  │ + execute(params, ctx)    │    │ + loadFromDirectory(dir)     │   │
│  └───────────────────────────┘    │ + reloadFromDirectory()      │   │
│                                   │ + reloadPlugin(path)         │   │
...
│                                   └──────────────────────────────┘   │
│                                                                      │
│  插件 .so（如 shell_plugin.so）导出 plugin_abi.h 符号；              │
│  业务逻辑可在同 .so 内用 C++ 实现，经 C execute 回调桥接。            │
└──────────────────────────────────────────────────────────────────────┘
```
...
> 完整的 Mermaid 时序图另存为：`docs/时序图.mermaid`
...
main()
  ...
  │     └── 创建 N 个 worker 线程
  │
  ├──4. ToolManager + 内置 read_file 注册
  │
  ├──5. PluginManager::loadFromDirectory(plugins/lib/)
  │     └── dlopen → ABI 校验 → init(host_api) → register()
  │     └── 宿主 register_tool → CallbackTool → ToolManager
  │
  ├──6. Dispatcher dispatcher
  │     └── 注册 MCP 方法 (tools/list, tools/call, plugins/reload)
  │
  ├──7. TcpServer server(...)
...
SIGINT/SIGTERM
  ...
  │     └── join 所有 worker 线程
  │
  ├──4. PluginManager::unloadAll(tool_manager)
  │     └── unregisterTool → mcp_plugin_shutdown() → dlclose
  │
  ├──5. Logger::shutdown()
        └── AsyncLogger::stop()
        └── 等待后台线程写完最后一批日志
...
| U-4 | 插件 ABI 稳定性 | 纯 C `plugin_abi.h` + `SOLARMCP_PLUGIN_ABI` 版本号；不跨 `.so` 传 C++ 对象 | ABI 结构体变更须版本 +1 并重新编译所有插件 |
...
```
# Plugin 系统（纯 C ABI）
include/mcp/plugin/plugin_abi.h              # C ABI 契约（插件唯一依赖的主程序头）
include/mcp/plugin/plugin_manager.h         # PluginManager 定义
include/mcp/plugin/callback_tool.h          # CallbackTool 定义
src/plugin/plugin_manager.cpp               # PluginManager 实现（dlopen/dlsym/validate/reload）
src/plugin/callback_tool.cpp                # CallbackTool 实现
plugins/CMakeLists.txt                      # 插件构建规则
plugins/example/minimal_plugin.c            # 最小插件示例（纯 C）
plugins/shell/shell_plugin.cpp              # shell 插件 C ABI 桥接
plugins/README.md                           # 插件目录说明
docs/插件ABI.md                             # ABI 设计与编写指南
docs/插件热重载.md                          # 热重载操作指南
...
**验收标准**:
1. `PluginManager::loadFromDirectory("./plugins/lib/")` 正确加载所有 `.so` 插件
2. 插件通过 C ABI `register_tool` 注册的工具出现在 `ToolManager`（经 `CallbackTool`）
3. 加载时校验 `mcp_plugin_abi_version()`，记录 `mcp_plugin_version()` / `mcp_plugin_name()`
4. `tools/list` 返回所有 Tool（内置 read_file + 插件 shell、plugin_demo 等）
5. 启动时自动扫描 `plugins/lib/`；退出时 `unloadAll`（unregister → shutdown → dlclose）
6. 热重载先 validate 再替换；validate 失败保留旧插件
7. GoogleTest 全部通过 (`ctest --test-dir build`)
8. 至少 5 个测试文件，覆盖核心模块
9. TimerWheel 正确添加/取消/触发定时器
10. 端到端：启动服务 → MCP Client 连接 → tools/list → tools/call → 正确响应
...
| `ToolManager` | 热重载时由 PluginManager 修改 | 平时只读；`unloadAll` / `reloadFromDirectory` 会 `unregisterTool` 并重新注册 |
| `PluginManager` | 非线程安全 | 仅在主线程加载/热重载；与 `tools/call` 并发无锁 |
...
插件与主程序通过 **纯 C ABI**（`include/mcp/plugin/plugin_abi.h`）通信，详见 [插件ABI.md](./插件ABI.md)。

| 约定 | 说明 |
|------|------|
| 插件依赖 | 仅 `#include "mcp/plugin/plugin_abi.h"`，不链接 `libsolarmcp.a` |
| 宿主适配 | `CallbackTool` 包装 C `execute` 回调后注册到 `ToolManager` |
| 版本校验 | `mcp_plugin_abi_version()` 必须等于 `SOLARMCP_PLUGIN_ABI` |
| 卸载顺序 | `unregisterTool` → `mcp_plugin_shutdown()` → `dlclose` |
| 热重载 | 先 `validatePlugin` 试加载，全部通过后再替换 |
| 配置路径 | 宿主经 `get_config_path` 传入 `config.yaml`，插件读取 `tools.*` |
